...
fn main() {
    ...
    // Precompute colours
    let colours = precompute_complex(MAX_ITERS as usize);
...
fn precompute_complex(iters: usize) -> Vec<u32> {
    let mut colours = vec![0; iters + 1];
    let a = 0.1;
    for (i, colour) in colours.iter_mut().enumerate() {
        ...
        *colour = from_rgb(r as u8, g as u8, b as u8);
    }
    colours
}
